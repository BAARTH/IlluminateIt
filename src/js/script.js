 ...
 var html_come_on = document.querySelector('.come_on');
 var light_loading = 0;
 ...
 function frameLooperLoading() {
     if (array_come_on.length > 0) {
         html_come_on.innerHTML += array_come_on.shift();
     } else {
         clearTimeout(timer_come_on);
     }
 ...
 function disable_loading() {
     {
         ...
         document.querySelector(".come_on_loading").className += " disable";
         document.querySelector(".skip_intro").className += " disable";
         next_to_loading = document.querySelectorAll(".next_to");
         for (i = 0; i < next_to_loading.length; i++) {
             next_to_loading[i].className += " disable";
         }
 ...
 var html_mooners = document.querySelector('.mooners');
 ...
 if (localStorage.getItem("First play") == null) { // If the page is visited for the first time
     ...
     var convert_rate = 10;
     var mooners_nbr = 1;
     var mooners_ratio = Math.log(Math.round(((mooners_nbr * 100) / product_increment) / 10));
     var capa_lumen_max;
     var nebula;
     var cx = 150;
     var cy = 150;
     ...
     var star_color = 0;
     var eclipse_spawn_interval = 20 * mooners_ratio;
     ...
     var star_color = parseInt(localStorage.getItem("star_color"));
     var eclipse_progress = parseInt(localStorage.getItem("eclipse_progress"));;
 ...
 function start() {
     ...
     html_light.innerHTML = 'Star brightness : ' + Math.round(light * 100) + '%';
     html_illuminers.innerHTML = Math.round(product_increment) + ' Illuminers';
     html_mooners.innerHTML = Math.round(mooners_nbr) + ' Mooners';
     ...
     localStorage.getItem("star_color", star_color);
     localStorage.getItem("eclipse_progress", eclipse_progress);
     localStorage.getItem("eclipse_durability", eclipse_durability);
 }
 redraw_on_buy();
 start();
 ...
 // MOONERS AND ECLIPSES

 function mooners_spawn() {
     mooners_nbr += Math.pow((Math.round(product_increment / 1000)), mooners_ratio);
 }

 //Calling mooners spawn every 3 minutes

 setInterval(mooners_spawn(), 180000);

 //Automatic generation of eclipse event

 setInterval(function() {

     var tirage = Math.round(Math.random() * 100);

     if (tirage <= eclipse_spawn_interval) {
         eclipse_progress += 1;
         console.log("An eclipse is coming !")
     };
     if (eclipse_progress == 10) {
         eclipse_spawn();
         eclipse_progress = 0;
     }
 }, 300000);

 function eclipse_spawn() {

     document.querySelector(".eclipse").style = 'display: block';
 }

 function eclipse_despawn() {

     document.querySelector(".eclipse").style = 'display: none';
     document.querySelector('.eclipse').classList.remove('eclipse_spawn');
     setTimeout(function() {
         document.querySelector(".eclipse").classList.add('eclipse_transition');
     }, 40);
 }

 document.querySelector(".eclipse").addEventListener('click', function() {
     click_sound.play();
     console.log(eclipse_durability)
     eclipse_durability += (-1);
     if (eclipse_durability == 0) {
         eclipse_despawn();
         eclipse_durability = 80;
     }
 })
 ...
 document.querySelector('.help_btn').addEventListener('click', function() {
     click_sound.play();
     if (help_status == 1) {
         document.querySelector('.help_container').style.transform = 'translateY(150%) translateX(-50%)';
         help_status = 0;
     } else if (help_status == 0) {

         document.querySelector('.help_container').style.transform = 'translateY(0) translateX(-50%)';
         help_status = 1;
     }
 })
